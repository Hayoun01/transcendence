...
export default async (fastify) => {
    ...
    fastify.get('/friends', async (request, reply) => {
        ...
        return friends.map((friendship) => ({
            id: friendship.id,
            userId: friendship.requester.id === userId ? friendship.receiver.id : friendship.requester.id,
            username: friendship.requester.id === userId ? friendship.receiver.username : friendship.requester.username,
    ...
    fastify.patch('/friends/:friendshipId/:action', async (request, reply) => {
        ...
        if (!friendship)
            return sendError(reply, 400, 'NO_SUCH_FRIENDSHIP_ID')
        if (action === 'accept') {
